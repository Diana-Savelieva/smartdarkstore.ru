...
${formData.message || 'Не указано'}
   `.trim();
    ...
    if (sendgridKey) {
      ...
      async function writeAndRead(command) {
      ...
      try {
        ...
        // Переопределяем encoder/decoder для нового TLS-соединения
        // (хотя они должны работать и так, но перестрахуемся)
        // Можно переопределить в принципе, но те же объекты можно использовать
